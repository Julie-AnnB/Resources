...
The current dataset comprised 2,321 observations of fatal car accidents
involving 1995 and later models of Ford and other SUVs. The data were
collected in the United States between 1995 and 1999 and excluded cases
in which vehicles were struck by another car or where the driver was
intoxicated. The number of passengers was used as a proxy for vehicle
load, while vehicle age served as a proxy for tire age (Ramsey &
Schafer, 2013).
...
The analysis was conducted in RStudio (2024.12.0) using R (version
4.5.1). The following functions and packages were used in the analysis:

- `table()` – to create a frequency table and calculate odds.

- `factor()` – to reorder the levels of the `Make` variable.

- `fisher.test()` – to calculate odds ratios and their confidence
  intervals.

- `barplot()` – to visualise the odds of for tire-related accidents
  between vehicle makes.  

- `plot()` for initial data exploration.

- `glm()` – to fit logistic regression models.

- `exp()` – to convert model coefficients from log-odds to odds ratios.

- `summary()` – to check model output and assess if the predictors were
  significantly associated with the outcome variable.

- `1 – pchisq()` – to evaluate the model fit using residual deviance.

- `step()` – to perform a selection of the best model.

- `anova()` – to compare which model fits.

- `confint()` – to measure the confidence intervals for the model
  coefficients.

- `predict()` – to create predicted probabilities from model using new
  data.

- `lines()` – to add fitted lines to plots.

- `ggplot2` – to create a visual representations of `Make`,
  `VehicleAge`, and `Passengers` variables in a single graph.

- Division and multiplication were used to determine if it was
  appropriate to calculate confidence intervals using the normal
  distribution.
...
Firstly, the levels of the `Make` variable were rearranged by setting
`Other` instead of `Ford` as the reference category (Step 1). This was
followed by a preliminary data exploration (Figure 1 and 2). In
particular, plots were used to examine the relationship between tire and
non-tire related accidents for Ford and other vehicle makes in relation
to the number of passengers and vehicle age.

An assumption check was performed to determine whether the dataset was
large enough and appropriate to use the normal approximation for
calculating the odds ratio confidence interval (CI) (Step 2). The 95% CI
for the odds ratio of tire-related accidents between Ford and other
vehicle makes was then calculated and visualised using a bar graph (Step
3, Figure 3).

Following this analysis, the raw data were collapsed from a binary to a
binomial dataset (Step 4). The revised dataset was visually inspected to
check if any log-transformations were needed (Figure 4 and 5). As no
transformations were performed, the data were fitted into a preliminary
logistic regression model referred to as Model_1, and coefficients were
exponentiated and interpreted in light of the study question (Step 5). A
deviance goodness-of-fit test assessed how well Model_1 performed in
predicting the outcome in using the observed dataset (Step 6).

A second model (Model_2) with interaction terms was fitted using
logistic regression, and an automatic backward stepwise selection
procedure was used to choose the best parameters. The suitability of
Model_2 was compared to Model_1, and the latter was selected as the
final model (Step 7). This was followed by calculating the 95%
confidence intervals for the model parameters, except the model
intercept (Step 8). Lastly, Model_1 was used to make predictions for
each combination of `Make`, `VehicleAge`, and `Passengers`. These
predictions were depicted using a graph, which was followed by a
Findings and Discussion section (Step 9, Figure 6).
...
![](Tire_Ford_files/figure-gfm/unnamed-chunk-4-1.png)<!-- -->
...
![](Tire_Ford_files/figure-gfm/unnamed-chunk-5-1.png)<!-- -->
...
![](Tire_Ford_files/figure-gfm/unnamed-chunk-11-1.png)<!-- -->
...
![](Tire_Ford_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->
...
![](Tire_Ford_files/figure-gfm/unnamed-chunk-14-1.png)<!-- -->
...
``` r
# I acknowledge the use of https://chat.openai.com for this graph. 
# My instruction included: "Why doesn't my code work for this graph?"
# This prompt was used to fix a graph that is suggested in week 6 of KMA253 lab report.

library(ggplot2)

pass_levels <- c(0, 2, 6, 8, 10)
newdata2 <- expand.grid(VehicleAge = ages,Passengers = pass_levels,Make =levels(ex2018$Make))

newdata2$pred_prob <- predict(fit, newdata = newdata2, type = "response")

ggplot(newdata2, aes(x = VehicleAge, y = pred_prob, colour = Make)) +  
  geom_line(linewidth = 0.7) + 
  scale_colour_manual(values = c("Ford" = "blue", "Other" = "red")) + 
  facet_wrap(~ Passengers, nrow = 1,labeller = label_both) +
  labs(x = "Vehicle age (years)", 
  y = "Probabilites for tire-related accidents", colour = "Make",title = "") + 
...
![](Tire_Ford_files/figure-gfm/unnamed-chunk-31-1.png)<!-- -->
